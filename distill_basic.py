...
class TeacherStudentDistillNet(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            if x.shape[2] != self.opt.teacher_input_image_size:
                ...
                teacher_x = x
            self.teacher_logit = self.teacher_model(
                teacher_x)
    ...
    def studentLossFn(self, student_pred, y, T, alpha):
        """
        ...
        Return: loss
        """
        prob_logit = F.log_softmax(student_pred, dim=1)
        target = F.softmax(self.teacher_logit, dim=1)
        # original
        logit_loss = -(target * prob_logit).sum(dim=1).mean()
        # loss_kl = F.kl_div(F.log_softmax(student_pred / T, dim=1), F.softmax(self.teacher_logit / T, dim=1),
        #                 reduction='batchmean')
        loss_cross_entropy = F.cross_entropy(student_pred, y)
        loss = logit_loss + loss_cross_entropy
        return logit_loss, loss_cross_entropy, loss
...
def get_optimizer(model, opt):
    params = split_weights(model.student_model)
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, opt, num_train_samples, no_acc_eval=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss_kl, loss_cross_entropy, loss = model.studentLossFn(student_pred=output, y=transformed_target, T=opt.T, alpha=opt.teacher_logit_weight)
        ...
        if i % opt.print_freq == 0 and opt.rank == 0:
            logging.info('Train epoch={}, i={}, loss={:4g},'
                         'logit_loss={:4g}, ts_feature_loss={:4g},'
                         'acc1={:4g}%, acc5={:4g}%, lr={:4g}'.format(
                epoch, i, float(loss),
                float(loss_cross_entropy), float(loss_kl),
