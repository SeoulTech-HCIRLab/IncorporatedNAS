...
efficient_score=0.5
population_size=256
evolution_max_iter=96000  # we suggest evolution_max_iter=480000 for


save_dir=CombineNAS/cifar_params1M_2
mkdir -p ${save_dir}
...
python train_image_classification.py --dataset cifar10 --gpu 1 --num_classes 10 \
...
python train_image_classification.py --dataset cifar100 --gpu 1 --num_classes 100 \
  --dist_mode single --workers_per_gpu 6 \
  --input_image_size 32 --epochs 1440 --warmup 5 \
  --optimizer sgd --bn_momentum 0.01 --wd 5e-4 --nesterov --weight_init custom \
  --label_smoothing --random_erase --mixup --auto_augment --shuffle_train_data \
