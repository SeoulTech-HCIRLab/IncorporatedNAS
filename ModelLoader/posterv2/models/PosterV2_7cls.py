...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768):
        ...
        face_landback_checkpoint = torch.load(r'/home/trangpi/Project/tiny-ml/ZenNAS_FER/ModelLoader/posterv2/models/pretrain/mobilefacenet_model_best.pth.tar',
        ...
        ir_checkpoint = torch.load(r'/home/trangpi/Project/tiny-ml/ZenNAS_FER/ModelLoader/posterv2/models/pretrain/ir50.pth', map_location=lambda storage, loc: storage)
    ...
    def forward(self, x):
        ...
        x_ir1, x_ir2, x_ir3 = self.ir_back(x, True)
    ...
    ### code for Teacher model mode @JinniPi
    def extract_stage_features_and_logit(self, x, target_downsample_ratio=None):
        ...
        o = torch.cat([o1, o2, o3], dim=1)
        feature, logit = self.VIT._extract_stage_features_and_logit(o)
